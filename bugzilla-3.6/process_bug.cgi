...
# Mandatory checking for  Root Cause 
 if ($cgi->param('resolution') eq 'FIXED') {
        #check Root Cause set
          if ($cgi->param('cf_rootcause')  =~ /^\s*$/) {
                ThrowUserError("rootcause_required");
          }
        #check Defect Type set
          if ($cgi->param('cf_testtype')  =~ /NA/) {
                ThrowUserError("testtype_required");
          }
