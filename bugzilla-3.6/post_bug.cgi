...
#Check for buildid in form 'branchname/YEARMMDDHH'
if (!defined $cgi->param('cf_buildid') || $cgi->param('cf_buildid') !~ /\w+ \(\d{10}\)\.*/) {
        ThrowUserError("require_buildid");
}
#Check for Severity/Priority set to something other than NA
if ($cgi->param('bug_severity') =~ /NA/ || $cgi->param('priority') =~ /NA/) {
        ThrowUserError("require_sevpriority");
}

if (!trim($cgi->param('comment'))) {
    ThrowUserError("description_required");
}

if ($cgi->param('cf_testtype') =~ /NA/) {
                ThrowUserError("require_testtype");
}
